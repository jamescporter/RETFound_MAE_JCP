...
# Define the root folder containing all images
root_folder = r"D:\2024_PHD-DATA_CFP-imgs_cropped"

# Determine the number of workers (all cores but 2)
...
def process_image(subdir, file):
    ...
    try:
        ...
        img_gray_resized = cv2.resize(img_gray, (512, 512))
...
with Manager() as manager:
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures)):
            ...
            # Estimate time after processing 1000 images
            if processed_count == 1000:
                first_1000_time_end = time.time()
                time_per_image = (first_1000_time_end - first_1000_time_start) / 1000
                remaining_images = total_images - processed_count
                estimated_total_time = remaining_images * time_per_image / num_workers

                logging.info(f"First 1000 imgs took: {first_1000_time_end - first_1000_time_start:.2f} seconds.")
                logging.info(f"Estimated remaining time: {estimated_total_time / 3600:.2f} hours.")

            # Periodically save progress every 1000 images
